...
class Cedaro_Theme_Fonts {
	...
	public function __construct( Cedaro_Theme $theme ) {
		$this->theme             = $theme;
		$this->stylesheet_handle = $theme->prefix . '-style';
	...
	public function add_support() {
		...
		add_action( 'wp_ajax_cedaro-theme-fonts-editor-css', array( $this, 'output_editor_styles' ) );
	...
	public function enqueue_fonts() {
		$url = $this->get_google_fonts_url();
		if ( ! empty( $url ) ) {
			wp_enqueue_style( 'cedaro-theme-fonts-google', $url );
		}
		...
		$kit_id = get_theme_mod( 'cedaro_fonts_typekit_id', '' );
		wp_enqueue_script(
			'cedaro-theme-fonts-typekit',
	...
	public function load_typekit_fonts() {
		if ( wp_script_is( 'cedaro-theme-fonts-typekit', 'done' ) ) {
	...
	public function register_tinymce_settings( $settings ) {
		$settings['cedaroFontsTypekitId'] = get_theme_mod( 'cedaro_fonts_typekit_id', '' );
		return $settings;
	}
	...
	public function register_tinymce_plugin( $external_plugins ) {
		if ( $this->is_typekit_active() ) {
			$external_plugins['cedarofonts'] = $this->theme->get_library_uri( '/assets/js/tinymce-fonts.js' );
		}
	...
	public function add_editor_styles( $stylesheets ) {
		$stylesheets .= ',' . add_query_arg( 'action', 'cedaro-theme-fonts-editor-css', admin_url( 'admin-ajax.php' ) );
		return $stylesheets;
	}
	...
	public function customize_register( $wp_customize ) {
		$wp_customize->register_section_type( 'Cedaro_Theme_Customize_Section_Fonts' );

		$wp_customize->add_section( new Cedaro_Theme_Customize_Section_Fonts( $wp_customize, 'cedaro_fonts', array(
			'title'       => esc_html__( 'Fonts', 'promenade' ),
			'priority'    => 50,
		) ) );

		$wp_customize->add_setting( 'cedaro_fonts_typekit_id', array(
		...
		foreach ( $this->text_groups as $group ) {
			...
			$wp_customize->add_control( new Cedaro_Theme_Customize_Control_Font( $wp_customize, $id, array(
				'label'         => $group['label'],
				'description'   => $group['description'],
				'section'       => 'cedaro_fonts',
				'settings'      => $id,
	...
	public function enqueue_customizer_preview_assets() {
		wp_enqueue_script(
			'cedaro-theme-customize-preview-fonts',
		...
		wp_localize_script( 'cedaro-theme-customize-preview-fonts', '_cedaroThemeFontsPreviewSettings', array(
	...
	public function enqueue_customizer_controls_assets() {
		wp_enqueue_style(
			'cedaro-theme-customize-controls-fonts',
		...
		wp_localize_script( 'cedaro-theme-customize-controls-fonts', '_cedaroThemeFontsControlsSettings', array(
			'fonts' => $this->fonts,
			'l10n'  => array(
				'reset'       => esc_html__( 'Reset', 'promenade' ),
				'defaultFont' => esc_html__( 'Default Theme Font', 'promenade' ),
			),
		) );
	}
	...
	public function print_templates() {
		?>
		<script type="text/html" id="tmpl-cedaro-theme-fonts-control-font">
			<label>
				<# if ( data.label ) { #>
			...
			<div class="cedaro-theme-fonts-control-content"></div>
		</script>
		<?php
	}
	...
	protected function get_subsets() {
		 ...
		 */
		$subset = esc_html_x( 'no-subset', 'Add new subset (latin-ext)', 'promenade' );
